<div class="row">
    ...
    <div class="col-md-6">
        <div class="form-group @error('customer_id') has-error @enderror">
            ...
            <select name="customer_id" id="customer_id" class="form-control select2" required onchange="getBalance(0)">
                <option value="">Select Customer</option>
                @php($customer_id = old('customer_id', isset($edit) ? $data->customer_id : ''))
                @foreach ($customers as $item)
                    <option value="{{ $item->id }}" {{ $customer_id == $item->id ? 'selected' : '' }}> {{ $item->name . ' - ' . $item->contact_no . ' - ' . $item->address }}</option>
                @endforeach
            </select>
        ...
        <table class="table table-bordered">
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="text-right">
                        <input type="text" class="form-control" id="total_amount" name="total_amount" value="{{ isset($edit) ? $data->transactions->sum('amount') : '' }}" readonly />
                    </td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
