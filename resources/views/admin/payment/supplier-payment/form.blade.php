<div class="row">
    ...
    <div class="col-md-6">   
        <div class="form-group @error('supplier_id') has-error @enderror">
            ...
            <select name="supplier_id" id="supplier_id" class="form-control select2" required onchange="getBalance(0)">
                <option value="">Select Supplier</option>
                @php($supplier_id = old('supplier_id', isset($edit) ? $data->supplier_id : ''))
                @foreach ($suppliers as $item)
                    <option value="{{ $item->id }}" {{ $supplier_id == $item->id ? 'selected' : '' }}> {{ $item->name . ' - ' . $item->contact_no . ' - ' . $item->address }}</option>
                @endforeach
            </select>
            @error('supplier_id')
                <span class="help-block">{{ $message }}</span>
