...
class LoanHolderService
{
    public static function due($id)
    {
        ...
        ->leftJoin(DB::raw("(SELECT loan_holder_id, SUM(amount) AS inAmount FROM `loans` WHERE type ='Payment' GROUP BY loan_holder_id) AS A"), function ($q) {
            $q->on('A.loan_holder_id', '=', 'loan_holders.id');
        })
        ->leftJoin(DB::raw("(SELECT loan_holder_id, SUM(amount) AS outAmount FROM `loans` WHERE type != 'Payment' GROUP BY loan_holder_id) AS B"), function ($q) {
